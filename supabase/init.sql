-- ============================================================
-- Swapill clean init schema
-- Synthesized from app code requirements (the chained migrations
-- conflict with each other). Idempotent: safe to re-run.
-- ============================================================

-- Reset public schema for a clean slate
...
-- Inline REFERENCES above auto-names the constraint skills_user_id_fkey,
-- which the app relies on for PostgREST embeds: skills!skills_user_id_fkey(*).
...
CREATE UNIQUE INDEX swap_requests_unique_pending
  ON public.swap_requests (sender_id, receiver_id)
...
CREATE POLICY "Participants can delete swap requests"
...
CREATE INDEX conversations_p1_idx ON public.conversations(participant_one);
CREATE INDEX conversations_p2_idx ON public.conversations(participant_two);
...
CREATE POLICY "Senders can insert their messages"
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, name, full_name, email, created_at, updated_at)
  VALUES (
    NEW.id,
    COALESCE(
...
-- ============================================================
-- Storage bucket: avatars (used for both avatars and cover photos)
-- ============================================================
