-- POPULATE DATABASE WITH 40 USERS - Complete Insertion Script
-- This script will insert Maryam Khaled with real UID and 39 expert profiles

-- Step 1: Insert Maryam Khaled with real Auth UID using upsert
...
-- Step 2: Insert 39 expert profiles with unique UUIDs
INSERT INTO public.profiles (id, user_id, full_name, bio, avatar_url, rating, swaps_count, created_at, updated_at) VALUES
...
-- Expert 24: Ali Hassan
(gen_random_uuid(), gen_random_uuid(), 'Ali Hassan', 'DevOps engineer optimizing deployment pipelines and infrastructure', 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=400&h=400&fit=crop&crop=face', 4.9, 28, NOW(), NOW()),
-- Expert 25: Salma Mahmoud
...
-- Step 4: Insert skills for all 39 experts
...
BEGIN
  ...
  -- Count Maryam's skills
  SELECT COUNT(*) INTO maryam_skills FROM public.skills 
  WHERE profile_id = (SELECT id FROM public.profiles WHERE user_id = 'e08b5109-a903-4fd3-8979-a5707fb11cbe');
  
  RAISE NOTICE '=== DATABASE POPULATION RESULTS ===';
  ...
  RAISE NOTICE 'Maryam skills: %', maryam_skills;
  ...
  IF expert_profiles = 39 THEN
    RAISE NOTICE '✅ SUCCESS: All 39 expert profiles created';
  ELSE
  ...
  IF total_skills >= 80 THEN
    RAISE NOTICE '✅ SUCCESS: Sufficient skills created (%)', total_skills;
  ELSE
    RAISE NOTICE '❌ ISSUE: Expected at least 80 skills, found %', total_skills;
  END IF;
  
  IF maryam_skills = 3 THEN
    RAISE NOTICE '✅ SUCCESS: Maryam has 3 skills';
  ELSE
    RAISE NOTICE '❌ ISSUE: Maryam should have 3 skills, found %', maryam_skills;
  END IF;
END $$;
